...
// Have to do this: not template only on Reg_Type. Then the Registration is not templated on the Reg_Type. Otherwise we can't separate these between the Model and Data_Set.

template<typename Registration, Reg_Type reg_type>
struct
Registry : Registry_Base {
};

struct Parameter_Registration : Registration_Base {
	//...
...
struct
Series_Set {
	//std::string file_name;  // Should not be needed.
...
Data_Set : Catalog {
	...
	// The step unit could be stored as a registration, but we are ever only going to want to have one.
