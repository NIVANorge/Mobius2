...
struct Dlib_Optimization_Model : public Optimization_Model {
	...
	Dlib_Optimization_Model(Model_Data *data, Expr_Parameters &parameters, std::vector<Optimization_Target> &targets, const std::vector<double> *initial_pars = nullptr, const Optim_Callback &callback = nullptr, s64 ms_timeout = -1)
	...
	double operator()(const column_vector &par_values) {
		...
		double result = evaluate(par_vals);
