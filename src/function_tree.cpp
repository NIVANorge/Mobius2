...
Math_Expr_FT *
binop_reduction_first_pass(Math_Expr_FT *expr, Function_Scope *scope) {
...
Math_Expr_FT *
binop_reduction_second_pass(Math_Expr_FT *expr, Function_Scope *scope) {
...
prune_tree(Math_Expr_FT *expr, Function_Scope *scope) {
	...
	switch(expr->expr_type) {
		...
		case Math_Expr_Type::binary_operator : {
			...
			auto result = binop_reduction_first_pass(expr, scope);
			if(result->expr_type == Math_Expr_Type::binary_operator)
				result = binop_reduction_second_pass(result, scope);
			if(result->expr_type == Math_Expr_Type::binary_operator)
				result = binop_reduction_first_pass(result, scope);  // In case the second pass reduced it to e.g. (0 + a)
