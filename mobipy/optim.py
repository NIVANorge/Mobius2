...
def run_minimizer(app, params, set_params, residual_fun, method='nelder', use_init=True, run_timeout=-1, disp=False) :
    ...
    def get_residuals(pars) :
        ...
        if success :
            ...
            sim[~np.isfinite(sim)] = very_large_number # To cause the run to be discarded if there were nan simulated values.
            resid = sim - obs
            
            del data
            if any(~np.isfinite(sim)) : 
                return very_large_number
            return float(np.nansum(np.square(resid)))
        else :
            #resid = np.array([very_large_number]*steps)
            #resid[0]=0
            del data
            return very_large_number
